...
async findAll(
  ...
  return await this.horasExtrasService.findByUserWithFilters(userId, filtros);
}
  ...
  async findOne(@Param('id') id: number, @Req() req: RequestWithUser) {
    const userId = req.user.userId;
    console.log('>>> [HorasExtrasController.findOne] Buscando hora extra:', { id, userId });
    
    if (!userId) {
      throw new Error('No se encontró el ID de usuario en req.user');
    }
    
    // ✅ Validar que el usuario solo pueda ver sus propias horas extras
  ...
  async updateEstado(
    ...
    // ✅ Validar que el usuario solo pueda cambiar el estado de sus propias horas extras
    const horaExtra = await this.horasExtrasService.findOne(id, userId);
    if (horaExtra.usuarioE !== userId) {
      throw new ForbiddenException('No tienes permisos para cambiar el estado de esta hora extra');
    }

    return this.horasExtrasService.updateEstado(id, updateEstadoDto.estado, userId);
  }
}